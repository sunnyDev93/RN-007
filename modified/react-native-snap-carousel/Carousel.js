...
export default class Carousel extends Component {
    ...
    _getContainerInnerMargin (opposite = false) {
        const { sliderWidth, sliderHeight, itemWidth, itemHeight, vertical, activeSlideAlignment, inactiveSlideScale } = this.props;

        if ((activeSlideAlignment === 'start' && !opposite) ||
            (activeSlideAlignment === 'end' && opposite)) {
                return vertical ? (sliderHeight - itemHeight) / 2 - 5 : (sliderWidth - itemWidth) / 2  - (sliderWidth - itemWidth) / 10;
            // return 0;
    ...
    _getViewportOffset () {
        ...
        if (activeSlideAlignment === 'start') {
            return vertical ? itemHeight / 2 : itemWidth;
