...
export default class ModalDropdown extends Component {
  ...
  _renderItem = ({ item, index, separators }) => {
    ...
    const preservedProps = {
      ...
      onPress: () => {
				if(member_plan == 3 || member_plan == 5 || member_plan == 6) {
					if(index > 1) {
						this._onRowPress(item, index, separators)
					}
				}
				else {
					this._onRowPress(item, index, separators)
    ...
    if (TOUCHABLE_ELEMENTS.find(name => name === row.type.displayName)) {
      ...
      switch (row.type.displayName) {
        case 'TouchableHighlight': {
          return <TouchableHighlight activeOpacity = {1} {...props}>{children}</TouchableHighlight>;
        }
        ...
        case 'TouchableWithoutFeedback': {
          return (
            <TouchableWithoutFeedback activeOpacity = {1} {...props}>
              {children}
            </TouchableWithoutFeedback>
          );
        }
        case 'TouchableNativeFeedback': {
          return (
            <TouchableNativeFeedback activeOpacity = {1} {...props}>
              {children}
            </TouchableNativeFeedback>
          );
        }
    ...
    return <TouchableOpacity activeOpacity = {1} {...preservedProps}>{row}</TouchableOpacity>;
  };
