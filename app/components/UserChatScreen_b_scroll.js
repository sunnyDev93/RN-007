...
var loadOldFlag = false;  // load old chat message or firebase message
var loadingOld = true;
var lastChatId = 0;
var imageSize = 36;
var profileUserId = '';
...
export default class UserChatScreen extends Component {
    ...
    */
    ...
    // callLoadMoreOldMessages = async () => {
    //     if (true) {
    //         loadingOld = false;
    //         console.log(TAG +" old result.data start old");
    //         try {
    //             let uri = Memory().env == "LIVE" ? Global.URL_GET_OLD_MESSAGE : Global.URL_GET_OLD_MESSAGE_DEV;
    //             console.log(TAG, " getOldMessages uri : ", uri);
    //             let result = await WebService.apiCallGeneralPost(uri);
    //             console.log(TAG + " old result getOldMessages ", result);
    //             if (result != false && result.status != null && result.status != undefined) {
    //                 if (result.status == "success") {
    //                     console.log("old result.data", result.data);
    //                 }
    //             }
    //             this.setState({
    //                 // dataChatMessage: msgData,
    //                 displayChatMessage: true,
    //                 displayLoadMoreView: true
    //             }, () => { setTimeout(() => this.autoScroll(true), 500); });
    //             this.setState({ deleteMessageLoading: false });
    //         } catch (error) {
    //             console.log(TAG, " getOldMessages error : ", error);
    //             if (error != undefined && error != null && error.length > 0) {
    //                 Alert.alert(error.replace(/<\/?[^>]+>/gi, "").replace(/\\n/g, "").replace(/\"/g, ""));
    //             }
    //         }
    ...
    handleCallLoadMoreMessages = async (messageList) => {
        ...
        let existing_msgData = this.state.dataChatMessage;
        for (let i = 0; i < existing_msgData.length; i++) {
            msgData.push(existing_msgData[i]);
        }
        this.setState({
            dataChatMessage: msgData,
            displayChatMessage: true,
    ...
    handleGetProfileDetailResponse = (response, isError) => {
        ...
        // console.log(" ------------- ", getfromAsyncstroage );
...
*/
        ...
        let listView = (
            <FlatList
                ...
                onScroll={(event) => {
                    ...
                    if (Math.floor(event.nativeEvent.contentSize.height - event.nativeEvent.layoutMeasurement.height) <= Math.floor(event.nativeEvent.contentOffset.y) && !this.state.deleteMessageLoading && event.nativeEvent.contentOffset.y > 20) {
    ...
    */
    ...
    handleSendMessagesNotificationResponse = (response, isError) => {
        // console.log(TAG + " callSendMessagesNotificationAPI Response " + JSON.stringify(response));
        console.log(TAG + " callSendMessagesNotificationAPI isError " + isError);
    };
    ...
    callSaveNewMessagesAPI = async () => {
        try {
            ...
            // body.append("token", this.state.userToken);
