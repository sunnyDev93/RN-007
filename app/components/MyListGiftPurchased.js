...
var TAG = "MyListGiftPurchased";

export default class MyListGiftPurchased extends React.Component {
    ...
    getPurchasedGiftList() {
        ...
        try {
            ...
            let uri = Memory().env == "LIVE" ? Global.URL_GIFTS_PURCHASED + this.state.viewall_page_number : Global.URL_GIFTS_PURCHASED_DEV + this.state.viewall_page_number;
            let params = new FormData();
            params.append("token", this.state.userToken);
            params.append("user_id", this.state.userId);
            params.append("format", "json");
            
            console.log(TAG + " callgetGiftsPurchased uri " + uri);
            console.log(TAG + " callgetGiftsPurchased params " + JSON.stringify(params));

            WebService.callServicePost(uri, params, this.handleGetGiftsPurchased);
        } catch (error) {
            console.log(TAG + " callgetGiftsPurchased error " + error);
            this.setState({
                loading: false
            });
    ...
    handleGetGiftsPurchased = (response, isError) => {
        console.log(TAG + " callgetGiftsPurchased Response " + JSON.stringify(response));
        console.log(TAG + " callgetGiftsPurchased isError " + isError);
    ...
    _renderGiftPurchasedItem = (item, index) => {
        // var imageUrl = item.imgpath + Constants.THUMB_FOLDER + item.filename;
        var imageUrl = item.image_path + item.image_name;
        
        return(
            <TouchableOpacity style={stylesGlobal.cardView} activeOpacity = {Global.activeOpacity} onPress = {() => this.gift_detail(item)}>
                ...
                <View style = {{width: '100%', marginTop: 15}}>
                    ...
                    <View style = {{width: '100%', marginBottom: 10, flexDirection: 'row'}}>
                        <Text style={[styles.rowText, stylesGlobal.font_bold]}>Purchased On: </Text>
                ...
                <View style = {{width: '100%', marginTop: 20, alignItems: 'center'}}>
                {
                    ...
                    <View style = {[styles.button]}>
                        <Text style = {[styles.buttonText, stylesGlobal.font]}>{item.shipping_status}</Text>
                    </View>
                }
                {
                    !item.shipping_status &&
                    <View style = {[styles.button]}>
                        <Text style = {[styles.buttonText, stylesGlobal.font]}>Ordered</Text>
                    </View>
                }
                </View>
            </TouchableOpacity>
            ...
            }
            {
                ...
                <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
                    <View style = {[stylesGlobal.empty_cardView, {height: '90%', justifyContent: 'center', alignItems: 'center'}]}>
                        <Text style={[stylesGlobal.card_empty_text, stylesGlobal.font, ]}>No Purchased Gifts</Text>
                        <TouchableOpacity style = {[stylesGlobal.common_button, stylesGlobal.shadow_style, {marginTop: 10}]} onPress = {() => this.props.screenProps.navigate("GiftList", {receiver: null})}>
                            <Text style = {[{fontSize: 14, color: Colors.white}, stylesGlobal.font]}>Buy Gift</Text>
                        </TouchableOpacity>
                    </View>
                </View>
            }
                ...
                <View style={{ flex: 1, marginLeft: 10, marginRight: 10, marginTop: 10 }}>
                    <FlatList
                        ...
                        renderItem={({ item, index }) => (
                            ...
                            {
                                this._renderGiftPurchasedItem(item, index)
                            }
                            </View>
                        )}
